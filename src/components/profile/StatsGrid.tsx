...
export const StatsGrid: FC<StatsGridProps> = ({ stats }) => {
  const getRarityDescription = (rarity: string) => {
    switch (rarity) {
      ...
      case 'Academy':
        return 'New player - Yet to play their first game';
      case 'Retired':
        return 'Inactive player with 0 XP (has played before)';
      default:
  ...
  const getFormIndicator = () => {
    ...
    if (difference > 0) {
      ...
      return (
        <span className="text-base-content/50" title="Same recent form">
          ⟷ 0.0%
        </span>
      );
    }
          ...
          {stat.tooltip ? (
            <TooltipPrimitive.Provider>
              <Tooltip content={stat.tooltip}>
                <h3 className="text-base-content/50 text-sm font-medium">{stat.label}</h3>
              </Tooltip>
            </TooltipPrimitive.Provider>
          ) : (
            <h3 className="text-base-content/50 text-sm font-medium">{stat.label}</h3>
