      ...
      >
              ...
              return {
                ...
                value: (
                  <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-2">
                    <span className="font-bold text-right whitespace-nowrap">{player?.maxWinStreak} wins</span>
                    {player?.maxStreakDate && (
                      <span className="text-xs opacity-80 text-right whitespace-nowrap">
        ...
        {(selectedYear === 'all' || selectedYear === new Date().getFullYear()) && (
          <AwardCard
            ...
            winners={stats.currentWinningStreaks.map(player => ({
              ...
              value: (
                <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-2">
                  <span className="font-bold text-right whitespace-nowrap">{player.currentWinStreak} wins</span>
                </div>
              ),
        ...
        <AwardCard
          ...
          winners={stats.topAttendanceStreaks.map(player => ({
            ...
            value: (
              <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-2">
                <span className="font-bold text-right whitespace-nowrap">{player.maxStreak} games</span>
        ...
        {(selectedYear === 'all' || selectedYear === new Date().getFullYear()) && (
          <AwardCard
            ...
            winners={stats.currentStreaks.map(player => ({
              ...
              value: (
                <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-2">
                  <span className="font-bold text-right whitespace-nowrap">{player.currentStreak} games</span>
                </div>
              ),
