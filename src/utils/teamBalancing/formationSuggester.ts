...
function getRatingBasedIdealPositions(player: TeamAssignment): PositionType[] {
  ...
  // PRIORITY 1: CHECK VERSATILITY FIRST - Versatile players should be in flexible positions
  if (isVersatilePlayer(player)) {
    positions.push("CM");  // Primary choice - uses all attributes
    if (defense > 5.5) positions.push("CDM");
    if (attack > 5.5) positions.push('CAM');
    return positions; // Early return for versatile players
  }
  // PRIORITY 2: CHECK PACE FOR WIDE POSITIONS - Fast players need dynamic roles
  if (attrs?.pace && attrs.pace >= 0.8) {
    if (defense > attack) {
      positions.push('WB'); // Fast defensive players are wingbacks
    } else {
      positions.push('W');  // Fast attacking players are wingers
    }
    if (gameIq > 7.0) positions.push('CM');
    if (attack > 6.5 && attrs?.shooting > 0.6) positions.push('ST');
    return positions; // Early return - pace is wasted at DEF
  }

  // CHECK HIGH GAME IQ - Tactical masterminds should be in central positions
  if (gameIq >= 9.0) {
    // Elite IQ (9.0+): Central positions ONLY, no DEF/wide roles
    positions.push('CM');  // Primary choice for orchestration
    // Only CDM if they have defensive capability
    if (defense > 5.0 && attrs && (attrs.defending >= 0.5 || attrs.physical >= 0.4)) {
      positions.push('CDM');
    }
    positions.push('CAM'); // Can create from advanced position
    // Early return - skip all other position logic for elite IQ
    return positions;
  }

  // Strong defensive orientation
  if (defense > attack + 1.0) {
    // Only add DEF if not high IQ and not fast (pace and intelligence wasted in pure defense)
    if (gameIq < 8.0 && (!attrs?.pace || attrs.pace < 0.7)) {
      positions.push('DEF');
    }
    // CDM requires actual defensive capability
    if (defense > 6.5 && gameIq > 6.0 && attrs && (attrs.defending >= 0.5 || attrs.physical >= 0.4)) {
      positions.push('CDM');
    }
  ...
  // Balanced player
    ...
    positions.push('CM');
    // CDM requires defensive capability
    if (defense > 6.0 && attrs && (attrs.defending >= 0.5 || attrs.physical >= 0.4)) {
      positions.push('CDM');
    }
  ...
  if (attrs) {
    // Only suggest DEF if strong defending AND not fast
    if (attrs.defending > 0.7 && defense >= attack && attrs.pace < 0.6 && !positions.includes('DEF')) {
      positions.push('DEF');
    }
  ...
  if (gameIq >= 8.0) {
    ...
    // Only CDM if they have defensive capability
    if (gameIq >= 8.5 && !positions.includes('CDM') && attrs && (attrs.defending >= 0.5 || attrs.physical >= 0.4)) {
      positions.push('CDM');
    }
    ...
    // Remove DEF position for high IQ players - tactical intelligence wasted in pure defense
    const defIndex = positions.indexOf('DEF');
    if (defIndex > -1) positions.splice(defIndex, 1);
 ...
 */
/**
 * Get debug info about formation selection
 */
function getFormationSelectionDebug(
  team: TeamAssignment[],
  requirements: RelativeRequirements,
  ...
  const wbCandidates = team.filter(player => {
    ...
    let idealPositions = [...new Set([...ratingBasedPos, ...playstylePos])];
    // Apply filtering
    const attrs = player.derived_attributes;
    if (isVersatilePlayer(player)) {
    ...
    if (attrs?.pace && attrs.pace >= 0.8) {
      ...
      if (player.defense_rating > player.attack_rating && !idealPositions.includes('WB')) {
        idealPositions.push('WB');
      }
    }
    return idealPositions.includes('WB') && (attrs?.pace ?? 0) >= 0.4;
  });

  // Count fast players
  const fastPlayers = team.filter(p => (p.derived_attributes?.pace ?? 0) >= 0.8);
  const versatilePlayers = team.filter(p => isVersatilePlayer(p));
  const highIqPlayers = team.filter(p => (p.game_iq_rating ?? 5) >= 8.0);

  return `WB: ${wbCandidates.length}, Fast: ${fastPlayers.length}, Versatile: ${versatilePlayers.length}, HighIQ: ${highIqPlayers.length}`;
}
...
function selectBestFormation(
  ...
  const attributeWbCandidates = team.filter(player => {
    ...
    const playstylePos = playstyle ? PLAYSTYLE_IDEAL_POSITIONS[playstyle] || [] : [];
    let idealPositions = [...new Set([...ratingBasedPos, ...playstylePos])];
    // Apply same filtering as in assignPlayersToPositions
    if (isVersatilePlayer(player)) {
      idealPositions = idealPositions.filter(p => p !== "DEF");
      if (!idealPositions.includes("CM")) idealPositions.push("CM");
    }
  ...
  const attributeWCandidates = team.filter(player => {
    ...
    if (attrs?.pace && attrs.pace >= 0.8) {
      idealPositions = idealPositions.filter(p => p !== "DEF");
      if (player.attack_rating > player.defense_rating && !idealPositions.includes("W")) {
  ...
  // Count versatile players who should be in central positions
  const versatilePlayers = team.filter(player => isVersatilePlayer(player)).length;

  // Count fast players (pace >= 0.8) who should NOT be at DEF
  const fastPlayers = team.filter(player => {
    const pace = player.derived_attributes?.pace ?? 0;
    return pace >= 0.8;
  }).length;
  ...
  if (versatilePlayers >= 3 || highIqCandidates >= 3) {
    // With many versatile or high IQ players, prefer central-heavy formations
  ...
  } else if (fastPlayers >= 2) {
    // With fast players, prefer formations with WB to utilize pace
    formationsToScore = [...validFormations].sort((a, b) => {
      // Prioritize WB formations for fast players
      const wbDiff = b.positions.WB - a.positions.WB;
      if (wbDiff !== 0) return wbDiff;
      // Avoid heavy DEF formations that waste pace
      return a.positions.DEF - b.positions.DEF;
    });
...
/**
 * Calculate attribute utilization for a position
 * Returns percentage of player's attributes that are actually used in this position
 */
function calculateAttributeUtilization(
  player: TeamAssignment,
  position: PositionType
): number {
  const attrs = player.derived_attributes;
  if (!attrs) return 1.0; // No penalty if no attributes
  ...
  const positionUsesAttribute: Record<PositionType, Set<string>> = {
    ...
    CDM: new Set(['defending', 'physical', 'passing', 'gameIq']),
    CM: new Set(['passing', 'dribbling', 'defending', 'pace', 'gameIq']),
    CAM: new Set(['passing', 'dribbling', 'shooting', 'pace', 'gameIq']),
    ST: new Set(['shooting', 'pace', 'dribbling', 'physical']),
  };

  const usedAttributes = positionUsesAttribute[position];
  let totalAttributeValue = 0;
  let usedAttributeValue = 0;

  // Calculate total vs used attribute values
  const attributeMap = {
    pace: attrs.pace,
    shooting: attrs.shooting,
    passing: attrs.passing,
  ...
  for (const [attrName, value] of Object.entries(attributeMap)) {
    if (value > 0.3) { // Only count significant attributes
      totalAttributeValue += value;
      if (usedAttributes.has(attrName)) {
        usedAttributeValue += value;
      }
    }
  }

  // Return utilization percentage (avoid division by zero)
  if (totalAttributeValue === 0) return 1.0;
  return usedAttributeValue / totalAttributeValue;
}

/**
 * Detect if a player is versatile (has many balanced attributes)
 */
function isVersatilePlayer(player: TeamAssignment): boolean {
  const attrs = player.derived_attributes;
  if (!attrs) return false;

  // Count attributes above 0.5
  let goodAttributes = 0;
  if (attrs.pace >= 0.5) goodAttributes++;
  if (attrs.shooting >= 0.5) goodAttributes++;
  if (attrs.passing >= 0.5) goodAttributes++;
  if (attrs.dribbling >= 0.5) goodAttributes++;
  if (attrs.defending >= 0.5) goodAttributes++;
  if (attrs.physical >= 0.5) goodAttributes++;

  // Versatile if 4+ attributes are good OR playstyle is "General" or "All-Rounder"
  const versatilePlaystyles = ['General', 'All-Rounder', 'Complete'];
  const playstyle = (player as any).playstyle_name || '';

  return goodAttributes >= 4 || versatilePlaystyles.some(style => playstyle.includes(style));
}
/**
...
function assignPlayersToPositions(
  ...
  outfieldPlayers.forEach(player => {
    ...
    if (attrs?.pace && attrs.pace >= 0.8) {
      ...
      if (player.defense_rating > player.attack_rating && !idealPositions.includes('WB')) {
        idealPositions.push('WB');
      } else if (!idealPositions.includes('W')) {
        idealPositions.push('W');
      }
    }
  ...
  sortedPlayers.forEach(([playerId, playerPositions]) => {
    ...
    for (const ps of playerPositions) {
      ...
      // Versatility bonuses - versatile players should be in central positions
      const isVersatile = isVersatilePlayer(player);
  if (isVersatile) {
        if (position === 'CM') priority += 3.0;  // Strong bonus for versatile CM
        if (position === 'CDM') priority += 2.0; // Good bonus for versatile CDM
        if (position === 'CAM') priority += 1.5; // Bonus for versatile CAM
        if (position === 'DEF') priority -= 2.0; // Penalty for wasting versatility at DEF
      }

      // STRONG pace penalties - fast players MUST NOT be at static positions
      if (attrs && attrs.pace >= 0.8) {
        if (position === 'DEF') {
          priority -= 5.0; // Massive penalty for elite pace at DEF
        }
        if (position === 'WB') {
          priority += 3.0; // Strong bonus for pace at WB
        }
        if (position === 'W') {
          priority += 2.5; // Strong bonus for pace at W
        }
      }
      }
      ...
      // High Game IQ players get VERY strong bonuses for central positions
      if (gameIq >= 9.0) {
        // Elite IQ (9.0+): Massive bonuses for central positions
        if (position === 'CDM') priority += iqAdvantage * 4.0; // Can orchestrate from deep
        if (position === 'CM') priority += iqAdvantage * 5.0; // Primary playmaker role
        if (position === 'CAM') priority += iqAdvantage * 4.0; // Creative hub

        // Strong penalties for non-central positions
        if (position === 'DEF') priority -= iqAdvantage * 2.0; // Huge penalty for pure defense
        if (position === 'WB' || position === 'W') {
          priority -= iqAdvantage * 2.5; // Massive penalty for wide positions
        }
      } else if (gameIq >= 8.0) {
        // High IQ (8.0-8.9): Strong bonuses for central positions
        if (position === 'CDM') priority += iqAdvantage * 2.5; // Defensive orchestrator
        if (position === 'CM') priority += iqAdvantage * 3.0; // Central playmaker
        if (position === 'CAM') priority += iqAdvantage * 2.5; // Attacking midfielder

        // Penalties for non-central positions
        if (position === 'DEF') priority -= iqAdvantage * 1.0; // Penalty for pure defense
        if (position === 'WB' || position === 'W') {
          priority -= iqAdvantage * 1.5; // Significant penalty for wasting intelligence
        }
      }
  ...
  playerScores.forEach(({ player, position, score, isNatural }) => {
    ...
    // CDM position constraints: need STRONG defensive capability and physical presence
    if (position === 'CDM') {
      // CDM requires both defensive ability AND physical/defensive attributes
      const hasDefensiveCapability = defense > 6.0 &&
                                     (attrs?.defending >= 0.5 || attrs?.physical >= 0.4);
      const hasMinimumDefense = defense > 5.5 && attrs?.defending >= 0.5 && attrs?.physical >= 0.3;
      const clearlyAttacking = attrs?.shooting > 0.7 && attack > defense + 1.0;

      // Must have either strong defense or decent defense with good attributes
      if (!(hasDefensiveCapability || hasMinimumDefense) || clearlyAttacking) {
        skipAssignment = true;
      }
    }
...
export function suggestFormations(
  ...
  const blueFormationDebug = getFormationSelectionDebug(blueTeam, requirements, blueFormation);
  const orangeFormation = selectBestFormation(orangeTeam, requirements);
  const orangeFormationDebug = getFormationSelectionDebug(orangeTeam, requirements, orangeFormation);
  ...
  debugLog.formationSelection.blue = {
    ...
    reasoning: `${blueComposition.attackingPlayers} attacking, ${blueComposition.defensivePlayers} defensive, ${blueComposition.balancedPlayers} balanced players. ${blueFormationDebug}`,
    teamComposition: blueComposition
  };

  debugLog.formationSelection.orange = {
    formation: orangeFormation.name,
    reasoning: `${orangeComposition.attackingPlayers} attacking, ${orangeComposition.defensivePlayers} defensive, ${orangeComposition.balancedPlayers} balanced players. ${orangeFormationDebug}`,
