-- Fix: Injury Token Not Applied in XP v2 Calculation
-- Issue: calculate_player_xp_v2() was not checking for injury_token_active
-- and using injury_return_streak for XP calculation while player is injured.
--
-- Affected players: Any player on injury reserve (injury_token_active = true)
-- Currently getting 0% streak bonus instead of their return streak bonus.
--
-- Solution: Check injury_token_active first, use injury_return_streak as effective streak.
-- Priority: injury_token > shield_token > current_streak
...
DECLARE
    ...
    -- New variables for injury token
    v_injury_active BOOLEAN;
    v_injury_return_streak INTEGER;
BEGIN
    ...
    -- ============================================
    -- INJURY TOKEN + SHIELD INTEGRATION
    -- Get current streak, injury status, AND shield status for calculations
    -- Priority: injury_token > shield_token > current_streak
    --
    -- If injury is active: use injury_return_streak directly
    -- If shield is active: use gradual decay formula
    -- Otherwise: use current_streak
    -- ============================================
    SELECT
        COALESCE(current_streak, 0),
        COALESCE(injury_token_active, false),
        COALESCE(injury_return_streak, 0),
        COALESCE(shield_active, false),
        protected_streak_value
    INTO v_current_streak, v_injury_active, v_injury_return_streak, v_shield_active, v_protected_streak
    FROM players
    WHERE id = p_player_id;

    -- Calculate effective streak
    -- Priority: injury (if active) > shield (if active) > current_streak
    IF v_injury_active THEN
        -- When injured, use return_streak for XP calculation
        -- This shows what their XP will be when they return
        v_effective_streak := v_injury_return_streak;
    ELSIF v_shield_active AND v_protected_streak IS NOT NULL THEN
    ...
    -- Uses v_effective_streak (accounts for injury/shield protection)
    -- 1 game: +10% (total: 10%)
    -- 2 games: +9% (total: 19%)
    -- 3 games: +8% (total: 27%)
    -- ...
