-- Fix merge_players to transfer shield columns from source to target
-- Root cause: merge transferred shield_token_usage and shield_token_history records
-- but did NOT copy player-level shield columns (shield_active, protected_streak_value, etc.)
-- This caused shield protection to vanish when merging a shielded player into a target.
...
BEGIN
    ...
    -- =====================================================
    -- Handle shield columns (transfer from source to target)
    -- =====================================================

    -- Copy shield columns from source to target if source has active shield
    IF EXISTS (SELECT 1 FROM players WHERE id = source_player_id AND shield_active = true) THEN
        UPDATE players SET
            shield_active = src.shield_active,
            protected_streak_value = src.protected_streak_value,
            protected_streak_base = src.protected_streak_base,
            shield_tokens_available = GREATEST(tgt.shield_tokens_available, src.shield_tokens_available),
            games_played_since_shield_launch = GREATEST(tgt.games_played_since_shield_launch, src.games_played_since_shield_launch)
        FROM players src, players tgt
        WHERE players.id = target_player_id
        AND src.id = source_player_id
        AND tgt.id = target_player_id;
    ELSE
        -- Even if shield not active, preserve the higher games_played count and tokens
        UPDATE players SET
            games_played_since_shield_launch = GREATEST(
                players.games_played_since_shield_launch,
                (SELECT games_played_since_shield_launch FROM players WHERE id = source_player_id)
            ),
            shield_tokens_available = GREATEST(
                players.shield_tokens_available,
                (SELECT shield_tokens_available FROM players WHERE id = source_player_id)
            )
        WHERE id = target_player_id;
    END IF;

    -- =====================================================
    -- END: Handle shield columns
    -- =====================================================
